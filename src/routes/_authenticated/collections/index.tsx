...
function Collections() {
    const collectionsQuery = useQuery(queries.collections.all);
    const isLoading = collectionsQuery.isPending;
    ...
    return (
        <section>
            ...
            {!isLoading && collectionsQuery.data?.length === 0 ? (
...
function CollectionSummary({ collection }: { collection: UserCollectionSummaryResponse }) {
    ...
    const unsubscribeMutation = useMutation({
        ...
        onSuccess: async () => {
            await queryClient.invalidateQueries({ queryKey: queries.collections.all.queryKey });
        },
    });

    const handleUnsubscribe = async (collectionId: string) => {
        await unsubscribeMutation.mutateAsync(collectionId);
    };

    const isMutating = unsubscribeMutation.isPending;
    ...
    return (
        <article key={collection.id} className="rounded-[1.75rem] border border-white/10 bg-white/4 p-6">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
                ...
                >
                    {isMutating ? 'Removing...' : 'Unsubscribe'}
                </button>
            </div>
        </article>
    );
